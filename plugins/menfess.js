...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!text) throw `*Cara penggunaan :*\n\n${usedPrefix + command} nomor|nama pengirim|pesan\n\n*Note:* nama pengirim boleh nama samaran atau anonymous.\n\n*Contoh:* ${usedPrefix + command} ${m.sender.split`@`[0]}|Holo|Ihacoy kak.`;
    let [jid, name, pesan] = text.split('|');
    if ((!jid || !name || !pesan)) throw `*Cara penggunaan :*\n\n${usedPrefix + command} nomor|nama pengirim|pesan\n\n*Note:* nama pengirim boleh nama samaran atau anonymous.\n\n*Contoh:* ${usedPrefix + command} ${m.sender.split`@`[0]}|Holo|Ihacoy.`;
    ...
    try {
        ...
        await conn.sendButton(data.jid, txt, wm, ['Oke', '.Oke'], null)
        .then(() => {
            m.reply('Berhasil mengirim pesan menfess, semoga di balas kak')
            conn.menfess[id] = {
                id,
                dari: m.sender,
                nama: name,

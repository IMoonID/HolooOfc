...
example ${usedPrefix}${command} limit 9 @62000000
...
`.trim()
    let c = wm
    conn.sendButton(m.chat, confirm, c, null, [['Y'], ['No']], m, { mentions: [who] })
...
handler.before = async m => {
    ...
    if (/(Tolak|n(o)?)/g.test(m.text.toLowerCase())) {
        clearTimeout(timeout)
        delete confirmation[sender]
        return m.reply('Reject')
    }
    if (/(Terima|y(es)?)/g.test(m.text.toLowerCase())) {
...
handler.help = ['otransfer', 'otf'].map(v => v + ' [type] [jumlah] [@tag]')
handler.tags = ['owner']
handler.command = /^(otf|ownertransfer|ownertf)$/i
handler.owner = true
